...
from archer.environment import (
    TwentyQuestionsEnv,
    BatchedTwentyQuestionsEnv,
    BatchedAdventureEnv,
    BatchedGuessMyCityEnv,
    BatchedWebShopEnv,
    BatchedGemmEnv,
)
from archer.models import ArcherAgent, CHAIAgent
from archer.algorithms import offpolicy_train_loop
from archer.prompts import (
    MISTRAL_TWENTY_QUESTIONS_TEMPLATE,
    mistral_twenty_questions_decode_actions,
)
...
def main(config: "DictConfig"):
    ...
    if config.env_name == "twenty_questions":
        ...
        eval_env = env
    if config.env_name == "gemm":
        print(">>> Using GEMM environment")
        env = BatchedGemmEnv(
            config.policy_lm,
            config.code_base_dir,
    ...
    if config.use_wandb and accelerator.is_main_process:
        ...
        wandb.init(
            project=config.project_name, name=config.run_name, config=dict(config)
        )
